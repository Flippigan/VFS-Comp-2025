...
class CircleDetectorNode(Node):
    ...
    def image_callback(self, msg):
        # Log every 100 camera frames to confirm we're getting data
        self.camera_data_count += 1
        if self.camera_data_count % 100 == 1:
            self.get_logger().info(f'Camera data received (frame {self.camera_data_count})')
        ...
        try:
            ...
            if circles is not None:
                ...
                # Log when circle is first detected or when properties change significantly
